...
let dataForecast = async(whichDayClicked) =>{
    const a = await data
    weatherDetailsInner.children[0].innerHTML = "Sunrise: "+a.forecast.forecastday[whichDayClicked].astro.sunrise + "<br>Sunset: " + a.forecast.forecastday[whichDayClicked].astro.sunset
    ...
    for(i=0;i<24;i++){
        ...
        //    weatherDetailsInner.children[1].innerHTML += "<span>0"+hour.getHours()+":00 &nbsp;"+a.forecast.forecastday[whichDayClicked].hour[i].temp_c + "°</span><br>"
            forecastInnerHtml = "<div id='span-hour'><span>0"+hour.getHours()+":00 &nbsp;"+a.forecast.forecastday[whichDayClicked].hour[i].temp_c + "°</span>"
        }
        else{
            forecastInnerHtml = "<div id='span-hour'>"+hour.getHours()+":00 &nbsp;"+a.forecast.forecastday[whichDayClicked].hour[i].temp_c + "°" 
        }
        weatherCondition(a.forecast.forecastday[whichDayClicked].hour[i].condition.code, 1, "forecast", forecastInnerHtml)
...
async function getData(userLocation) {
    ...
    weatherCondition(results.current.condition.code, results.current.is_day, "today")
    weatherCondition(results.forecast.forecastday[0].day.condition.code, results.current.is_day, "day1")
    weatherCondition(results.forecast.forecastday[1].day.condition.code, results.current.is_day, "day2")
    weatherCondition(results.forecast.forecastday[2].day.condition.code, results.current.is_day, "day3")
...
async function weatherCondition(code, isDay, whichDay, forecastInnerHtml) {
    ...
    if(whichDay != "forecast"){
       if(currentConditionImg == 332 || currentConditionImg == 320){
        ...
        else if(currentConditionImg == 116 || currentConditionImg == 117 || currentConditionImg == 119){
            for(i=0; i<3; i++){
    ...
    if(whichDay == "today"){
        weatherState.innerHTML = "<span>"+currentConditionText+"</span><img src='weathericons/"+currentConditionImg+".png'>" 
    }
        ...
        weatherDetailsInner.children[1].innerHTML += forecastInnerHtml + "<img src='weathericons/" + currentConditionImg + ".png'></div>"
    }
    else{
